...
I will use two datasets : one from Stéphane Robin's training (Barents Fish).
...
cr_sub <- sort(cr_sub, decreasing = TRUE)

# I have a weired bug, if I try to remove all the correlation == 1 (i.e the diagonal), I still have some 1 left...
# 
# cr_sub <- cr_sub[cr_sub != 1]
#
# To overcome it, I will remove the 30 first element of the vector, because it is a square matrix of 30*30.

cr_sub <- cr_sub[31:length(cr_sub)]
...
stringr::str_replace_all(network_gllvm$from, node_name_gllvm_vector)

fruits %>%
  stringr::str_replace_all(c("one" = "1", "two" = "2", "three" = "3"))
